...
public:
			  ...
			  uint32 onDate = 0)
	{
		...
		if (dbFolder)
		{
			...
			#ifdef __linux__
			pthread_t thread;
            int result=pthread_create(&thread, NULL, DniproDB::writeTrans, this);
			#endif
		}
	...
	{
		ulong64 usedMemory = sizeof(DniproDB); // +valueListPool.getTotalMemory();
	...
	{
		ulong64 totalMemory = sizeof(DniproDB); // +valueListPool.getTotalMemory();
