...
std::atomic<uint32> DniproDB::amountReaders;// = 0;
std::atomic<uint32> DniproDB::amountMarkIsReadedTrans;// = 0;
std::atomic<uint32> DniproDB::amountShapshotTrans;// = 0;

ulong64 DniproDB::tranLogSize = 0;
ulong64 DniproDB::blobLogSize = 0;

#ifdef _WIN32
CRITICAL_SECTION DniproDB::writeTranLock;
CRITICAL_SECTION DniproDB::writeBlobLock;
#endif

#ifdef __linux__
pthread_mutex_t DniproDB::writeTranLock;
pthread_mutex_t DniproDB::writeBlobLock;
#endif
...
#endif
#ifdef __linux__
void* DniproDB::writeTrans(void* lpParam)
#endif
{
...
bool DniproDB::checkDeadlock(uchar8 tranID)
{
	bool val = false;
...
void DniproDB::clearChilds(uint32 tranID)
{
	for (uint32 i = 1; i < MAX_TRANS; i++)
	{
...
void DniproDB::clearParts(uint32 tranID)
{
...
uint32 DniproDB::beginTran(uchar8 tranType,
						 uchar8 parentTranID)
{
	while (true)
	{
...
void DniproDB::rollbackTran(uint32 tranID)
{
	HArrayTran& tran = _trans[tranID];
...
void DniproDB::clearTran(uint32 tranID)
{
	_trans[tranID].clear(); //always readonly tran
}

void DniproDB::commitTran(uint32 tranID)
{
	HArrayTran& tran = _trans[tranID];
