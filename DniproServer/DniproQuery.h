...
public:
	...
	DniproQuery* getWhere(char* query, uint32 docID)
	{
		if (!TranID)
		{
			TranID = pDB->beginTran(READONLY_TRAN); //readonly tran
	...
	DniproQuery* andWhere(char* query, uint32 docID)
	{
		if (!TranID)
		{
			TranID = pDB->beginTran(READONLY_TRAN); //readonly tran
	...
	DniproQuery* orWhere(char* query, uint32 docID)
	{
		if (!TranID)
		{
			TranID = pDB->beginTran(READONLY_TRAN); //readonly tran
	...
	/*
	DniproQuery* print(char* query1, uint32 index1,
