...
void HArrayVarRAM::delValueByKey(uint* key,
								 uint keyLen,
								 uint value,
								 uint index)
{
...
NEXT_KEY_PART:
		...
		if(contentCellType >= ONLY_CONTENT_TYPE) //ONLY CONTENT =========================================================================================
		{
			...
			if(contentIndex < maxSafeShort) //content in one page
			{
				...
				{
					ValueList* pValueList = valueListPool.fromSerPointer(pContentPage->pContent[contentIndex].Value);
		...
		if(contentCellType == VAR_TYPE) //VAR =====================================================================
		{
			...
			else
			{
				...
				{
					ValueList* pValueList = valueListPool.fromSerPointer(varCell.ValueContentCell.Value);
					pValueList->delValue(value, index);
				}
		...
		else if(keyOffset == keyLen)
		{
			...
			else if(contentCellType == VALUE_LIST_TYPE)
			{
				ValueList* pValueList = valueListPool.fromSerPointer(contentCellValueOrOffset);
				pValueList->delValue(value, index);
			}
