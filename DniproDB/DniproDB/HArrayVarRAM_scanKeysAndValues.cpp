	...
	void* pData)
{
	...
	NEXT_KEY_PART:
		...
		if (contentCellType >= ONLY_CONTENT_TYPE) //ONLY CONTENT =========================================================================================
		{
			uint fullKeyLen = keyOffset + contentCellType - ONLY_CONTENT_TYPE;
									 ...
									 void* pData)
{
	uint keyOffset = 0;
	uint startHeader = 0;
	uint endHeader = 0xFFFFFFFF >> HeaderBits;

	//start range
	//middle range
	for (uint currKey = 0; currKey < endHeader; currKey++)
	{
		uint contentOffset = pHeader[currKey];
