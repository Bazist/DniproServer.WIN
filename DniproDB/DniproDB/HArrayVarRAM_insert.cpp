		...
		if(contentIndex < maxSafeShort) //content in one page
		{
			for(; keyOffset < originKeyLen; contentOffset++, contentIndex++, keyOffset++)
			{
				...
				if(keyOffset == keyLen)
				{
					...
					varCell.ContentCell.Type = contentCell.Type;
					varCell.ContentCell.Value = contentCell.Value;

					//shounted, notify trans !
					if (onContentCellMovedFunc)
					{
						uchar tranID = contentCell.ReadedByTranID.load();
						if (tranID)
						{
			...
			else //key is exists, update
			{
				...
				if(AllowValueList)
				{
					...
					{
						uint prevValue = contentCell.Value;
		...
		else  //content in two pages
		{
			for(; keyOffset < originKeyLen; contentOffset++, keyOffset++)
			{
				...
				if(keyOffset == keyLen)
				{
					...
					varCell.ContentCell.Type = currContentCell.Type;
					varCell.ContentCell.Value = currContentCell.Value;

					//shounted, notify trans !
					if (onContentCellMovedFunc)
					{
						uchar tranID = currContentCell.ReadedByTranID.load();
						if (tranID)
						{
							onContentCellMovedFunc(tranID,
			...
			contentCellType == VALUE_LIST_TYPE) //update existing value
	{
		...
		else
		{
			if(AllowValueList)
			{
				...
				{
					uint prevValue = pContentCell->Value;
