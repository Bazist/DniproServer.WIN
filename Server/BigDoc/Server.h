...
class Server
{
	public:
		...
		#endif
		{
			...
			if (iResult != NO_ERROR)
			{
				BigDocError::Print(CONNECTION_ERROR, "Error at WSAStartup().\n");
			...
			{
				#ifdef _WIN32
				BigDocError::Print(CONNECTION_ERROR, "Error at socket(): %ld.\n", WSAGetLastError());

				WSACleanup();
				#endif

				#ifdef __linux__
				BigDocError::Print(CONNECTION_ERROR, "Error at socket().\n");
				#endif
			...
			{
				BigDocError::Print(CONNECTION_ERROR, "bind() failed.\n");
			...
			{
				BigDocError::Print(CONNECTION_ERROR, "Server: Error listening on socket.\n");
			...
			BigDocInfo::Print("Server started (port 4477)...\n");
			...
			NEXT_CLIENT:
				...
				{
					#ifdef _WIN32
					BigDocError::Print(CONNECTION_ERROR, "ListenSocket error: %ld.\n", WSAGetLastError());
					#endif

					#ifdef __linux__
					BigDocError::Print(CONNECTION_ERROR, "ListenSocket error.\n");
					#endif
			...
			NEXT_PACKET:
				...
				if (bytesRecv != -1)
				{
					try
					{
						...
						while (fullBytesRecv > bytesRecv)
						{
							...
							{
								#ifdef _WIN32
								BigDocError::Print(CONNECTION_ERROR, "AcceptSocket error %ld.\n", WSAGetLastError());
								#endif

								#ifdef __linux__
								BigDocError::Print(CONNECTION_ERROR, "AcceptSocket error.\n");
								#endif
						...
						if (isSimpleExecPacket)
						{
							...
							BigDocInterpreter di(pDB, sendBuffer);
						...
						else
						{
							...
							{
								BigDocPacket* pPacket = (BigDocPacket*)(pBuffer + i);
								char* json = (pBuffer + i + sizeof(BigDocPacket));

								if (BigDocInterpreter::IsProfilerActive)
								{
									BigDocInfo::Print("%s\n", json);
								...
								case 16: //clear
								{
									...
									BigDocInfo::Print("Database cleared !\n");
								...
								case 30: //Exec
								{
									BigDocInterpreter di(pDB, sendBuffer + sendBufferSize);
								...
								default:
								{
									BigDocError::Print(CONNECTION_ERROR, "Server Method type is undefined.\n");

									break;
								}
								}

								i += (sizeof(BigDocPacket) + pPacket->QuerySize);
							}
						}
					}
					catch (BigDocError de)
					{
						de.Print();
				...
				{
					#ifdef _WIN32
					BigDocError::Print(CONNECTION_ERROR, "Server recv() error %ld.\n", WSAGetLastError());
					#endif

					#ifdef __linux__
					BigDocError::Print(CONNECTION_ERROR, "Server recv() error.\n");
					#endif
					break;
				}
				...
				if(sendBuffer)
				{
					...
					{
						#ifdef _WIN32
						BigDocError::Print(CONNECTION_ERROR, "Server send() error %ld.\n", WSAGetLastError());
						#endif

						#ifdef __linux__
						BigDocError::Print(CONNECTION_ERROR, "Server send() error.\n");
						#endif
