...
public:
	...
	void startTest(uint32 number, char* name)
	{
		start = msclock();
	...
	void endTest(uint32 amountQueries = 0,
				 uint32 controlValue = 0)
	{
		uint32 time = (uint32)(msclock() - start) + 1;

		printf("=> Time: %u ms.", time);

		if (amountQueries)
		{
			printf(" Queries/sec: %u", (uint32)(amountQueries * 1000 / time));
		}
	...
	void total()
	{
		...
		printf("=> [iCore 5, BigDoc v1_0_3]: 7994 ms\n");
	...
	void init()
	{
		...
		for (uint32 i = 0; i<TESTS_COUNT_KEYS; i++)
		{
	...
	void test3()
	{
		...
		uint32 tranID = pDB->beginTran();

		for (uint32 i = 1; i<=TESTS_COUNT_KEYS; i++)
		{
	...
	void test5()
	{
		...
		for (uint32 i = 1; i <= TESTS_COUNT_KEYS; i++)
		{
			temp += (ulong64)pDB->getDocsByAttr(query[i%100]);
		}
	...
	void test9()
	{
		...
		for (uint32 i = 1; i <= TESTS_COUNT_KEYS/1000; i++)
		{
			temp += (ulong64)pDB->getDocsByAttr(query[i % 100]);
