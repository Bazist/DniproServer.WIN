...
#ifndef _BigDoc_INTERPRETER		 // Allow use of features specific to Windows XP or later.                   
#define _BigDoc_INTERPRETER 0x712 // Change this to the appropriate value to target other versions of Windows.
...
class BigDocInterpreter
{
public:
	BigDocInterpreter(BigDoc* pDB,
		char* jsonResult = 0,
	...
	static void restartServer(BigDoc* pDB = 0,
		STOP_SERVER_FUNC* stopServer = 0,
		char* withParam = 0)
	{
		char exePath[512];
	...
	bool runQuery()
	{
		...
		try
		{
			BigDocQuery* pDQ = new BigDocQuery(pDB);
			pDQ->TranID = TranID;
					...
					case 'G':
					{
						...
						else if (!strcmp(method.MethodName, "GetInfo"))
						{
							if (method.ParamCount == 0)
							{
								...
								//BigDocInfo::Print("Version: %s\n", BigDoc_VERSION);
								BigDocInfo::Print("Amount documents: %u\n", pDB->lastDocID);
								BigDocInfo::Print("Amount collections: %u\n", pDB->countColls);
								BigDocInfo::Print("Total Memory (bytes): %u\n", pDB->getTotalMemory());
								BigDocInfo::Print("Tran Log size (bytes): %u\n", pDB->tranLogSize);
								BigDocInfo::Print("Tran Blob size (bytes): %u\n", pDB->blobLogSize);
								BigDocInfo::PrintLine();
							}
							else
							{
