...
public:
	/*
	Storage(BigDoc* pDB, char* folder)
	{
		this->pDB = pDB;
	...
	static DWORD WINAPI syncJob( LPVOID lpParam ) 
	{
		...
		uint32 logSize = pTranLog->getFileSize();
		...
		while(true)
		{
			...
			//	uint32 tm1 = pStorage->pDB->ha1.getTotalMemory();
			//	uint32 tm2 = pStorage->pDB->ha2.getTotalMemory();
			//	uint32 tm3 = pStorage->pDB->attrValuesPool.getTotalMemory();
	...
	{
		const uint32 buffSize = 10000000;
		...
		if(pFile->open())
		{
			...
			uint32 safeLen = buffSize - 1000000;
